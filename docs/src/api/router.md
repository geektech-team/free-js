...
Free-JS 路由以插件形式安装，并通过 `RouterView` 渲染当前路由组件。
...
```typescript
import { Component, VNode } from 'free-js';
import { RouterView } from 'free-js/router';
...
```typescript
import { RouterLink } from 'free-js/router';
...
const stop = router.onRouteChange((to, from) => {
  document.title = String(to.meta?.title ?? 'Free-JS');
});
