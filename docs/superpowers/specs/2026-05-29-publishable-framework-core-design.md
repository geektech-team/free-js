...
Each behavior change gets a failing Vitest test first. Package output also gets a consumer-style smoke test that builds, packs to a temporary cache, installs the tarball into a temporary project, imports `free-js` and `free-js/router`, and runs TypeScript against the consumer.
