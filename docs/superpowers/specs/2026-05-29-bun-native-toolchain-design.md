...
Free-JS should use Bun as its only project runtime/toolchain layer. Development, tests, library build, package smoke checks, and docs serving should run through Bun commands. The published library remains standard ESM plus declarations so JavaScript consumers can still import it normally.
...
- Temporary Bun consumer project imports `free-js` and `free-js/router` and passes `tsc --noEmit`.
