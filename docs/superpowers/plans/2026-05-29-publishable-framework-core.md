...
**Goal:** Make Free-JS publish/install reliably and add typed components, keyed diffing, real slots, and dynamic route params.
...
- [ ] Write a failing package smoke test that runs `npm run build`, `npm pack --dry-run --json`, checks that exported files exist, and verifies a temporary consumer can import `free-js` and `free-js/router`.
...
- [ ] Run `npm test`.
- [ ] Run `npm run build`.
- [ ] Run `npm_config_cache=/private/tmp/free-js-npm-cache npm pack --dry-run --json`.
